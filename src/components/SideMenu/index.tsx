...
const SideMenu = () => {
  ...
  const resolveActiveMenu = () => {
    return menus.filter((menue) => menue.name == resolveMenuFromRoute())
      .length > 0
      ? menus.filter((menue) => menue.name == resolveMenuFromRoute())[0]
      : menus[0];
  };
              ...
              return (
                <div key={i}>
                  <div
                    ...
                    data-mode={activeMenu.name == menu.name ? "active" : ""}
                    className={`${theme}-SideMenu-MenuList-menu-container`}
                  >
                    <img
                      data-mode={activeMenu.name == menu.name ? "active" : ""}
                      className={`${theme}-icons-${menu.icon}`}
                      alt=""
                    />
                  </div>
                </div>
